...
function ProfileCard({
  ...
  return ( // 此处的return是ProfileCard组件的返回值，表示该组件渲染的内容。它返回一个包含用户信息和交互元素的文章元素（<article>），其中包括头像、名字、职业、技能标签、简介、在线状态和关注按钮等。这个返回值将被React用来更新页面上的UI，以显示用户的个人资料卡片。
...
export default function App() { //export default function App() 定义了一个名为 App 的 React 组件，并将其导出为默认导出，使其可以在其他文件中被导入和使用。
  ...
  function handleClearSearch() {
    setSearchText(""); // 将 searchText 状态重置为空字符串，清除搜索输入框的内容。
  }
  ...
  const filteredProfiles = profiles.filter((profile) => {
    ...
    return ( // 此处的return 是 filter() 方法的回调函数的返回值，表示是否保留当前的 profile 对象。它检查 profile 的 name、role 和 skills 是否包含搜索关键词，如果包含则返回 true，表示保留该 profile；否则返回 false，表示过滤掉该 profile。最终 filteredProfiles 将包含所有匹配搜索条件的 profile 对象。
  ...
  return ( // 此处的 return 是 App 组件的返回值，表示该组件渲染的内容。它返回一个包含用户资料卡片和搜索功能的主元素（<main>），其中包括一个头部显示关注人数和搜索输入框，以及一个根据搜索结果过滤后的用户资料卡片列表。这个返回值将被React用来更新页面上的UI，以显示用户的个人资料和搜索功能。
    <main className="page">
